// src/config/templateConfig.ts

import type { TemplateId } from '../types/cv.types';

export interface TemplateConfig {
  id: TemplateId;
  label: string;
  description: string;
  targetUsers: string;
  /** Baked-in accent for the design picker swatch; template CSS defines the real palette. */
  templateAccent?: string;
  layout: 'single-column' | 'two-column';
  sidebarSections?: string[];
  sectionOrder: string[];
  educationFirst: boolean;
  showSkillBars: boolean;
  /** When true, the CV editor shows skill level (1–5) controls; the PDF uses bars/dots or clear level visuals. */
  showsSkillRatingInCv: boolean;
  showPhoto: boolean;
  hideIfEmpty: string[];
  requiredSections: string[];
}

/**
 * Every key handled by `renderMainSection` in `src/templates/shared/sections.js`.
 * Each template's `sectionOrder` must include each key exactly once so saved orders merge
 * correctly and no section is permanently omitted for that layout.
 */
export const CV_TEMPLATE_SECTION_KEYS_ALL: readonly string[] = [
  'personal',
  'summary',
  'experience',
  'projects',
  'education',
  'skills',
  'publications',
  'research',
  'certifications',
  'awards',
  'volunteer',
  'languages',
  'interests',
  'references',
  'custom',
];

export const TEMPLATE_CONFIGS: Record<TemplateId, TemplateConfig> = {
  classic: {
    id: 'classic',
    label: 'Classic',
    description: 'Clean single-column, ATS-optimized',
    targetUsers: 'Corporate, Finance, Law, Government',
    layout: 'single-column',
    sectionOrder: [
      'personal',
      'summary',
      'experience',
      'projects',
      'education',
      'skills',
      'certifications',
      'awards',
      'volunteer',
      'publications',
      'research',
      'languages',
      'interests',
      'references',
      'custom',
    ],
    educationFirst: false,
    showSkillBars: false,
    showsSkillRatingInCv: false,
    showPhoto: false,
    hideIfEmpty: ['publications', 'research', 'projects'],
    requiredSections: ['personal', 'summary', 'experience', 'education', 'skills'],
  },
  modern: {
    id: 'modern',
    label: 'Modern',
    description: 'Two-column with skills sidebar',
    targetUsers: 'Tech, Product, Marketing, Startups',
    layout: 'two-column',
    sidebarSections: ['skills', 'languages', 'certifications', 'interests'],
    sectionOrder: [
      'personal',
      'summary',
      'experience',
      'projects',
      'education',
      'publications',
      'research',
      'awards',
      'volunteer',
      'references',
      'custom',
      'skills',
      'certifications',
      'languages',
      'interests',
    ],
    educationFirst: false,
    showSkillBars: true,
    showsSkillRatingInCv: true,
    showPhoto: true,
    hideIfEmpty: ['publications', 'research'],
    requiredSections: ['personal', 'summary', 'experience', 'skills'],
  },
  academic: {
    id: 'academic',
    label: 'Academic',
    description: 'Dense, publications-first for researchers',
    targetUsers: 'PhD, Professors, Researchers, Postdocs',
    layout: 'single-column',
    sectionOrder: [
      'personal',
      'summary',
      'research',
      'publications',
      'education',
      'experience',
      'projects',
      'awards',
      'skills',
      'certifications',
      'volunteer',
      'languages',
      'interests',
      'references',
      'custom',
    ],
    educationFirst: false,
    showSkillBars: false,
    showsSkillRatingInCv: false,
    showPhoto: false,
    hideIfEmpty: ['projects', 'volunteer'],
    requiredSections: ['personal', 'education', 'publications', 'research'],
  },
  technical: {
    id: 'technical',
    label: 'Technical',
    description: 'Skills matrix + projects focus',
    targetUsers: 'Software Engineers, DevOps, Data Scientists',
    layout: 'two-column',
    sidebarSections: ['skills', 'certifications', 'languages'],
    sectionOrder: [
      'personal',
      'summary',
      'experience',
      'projects',
      'education',
      'publications',
      'research',
      'awards',
      'volunteer',
      'references',
      'custom',
      'skills',
      'certifications',
      'languages',
      'interests',
    ],
    educationFirst: false,
    showSkillBars: true,
    showsSkillRatingInCv: false,
    showPhoto: false,
    hideIfEmpty: ['publications', 'research', 'volunteer'],
    requiredSections: ['personal', 'skills', 'experience', 'projects'],
  },
  minimal: {
    id: 'minimal',
    label: 'Minimal',
    description: 'Sparse executive layout',
    targetUsers: 'C-Suite, Senior Directors, VPs',
    layout: 'single-column',
    sectionOrder: [
      'personal',
      'summary',
      'experience',
      'projects',
      'education',
      'skills',
      'publications',
      'research',
      'certifications',
      'awards',
      'volunteer',
      'languages',
      'interests',
      'references',
      'custom',
    ],
    educationFirst: false,
    showSkillBars: false,
    showsSkillRatingInCv: false,
    showPhoto: false,
    hideIfEmpty: [
      'projects',
      'publications',
      'certifications',
      'volunteer',
      'research',
    ],
    requiredSections: ['personal', 'summary', 'experience'],
  },
  creative: {
    id: 'creative',
    label: 'Creative',
    description: 'Visual layout with photo support',
    targetUsers: 'Designers, Artists, Writers, UX',
    layout: 'two-column',
    sidebarSections: ['skills', 'tools', 'languages', 'interests'],
    sectionOrder: [
      'personal',
      'summary',
      'projects',
      'experience',
      'education',
      'publications',
      'research',
      'certifications',
      'awards',
      'volunteer',
      'references',
      'custom',
      'skills',
      'languages',
      'interests',
    ],
    educationFirst: false,
    showSkillBars: true,
    showsSkillRatingInCv: true,
    showPhoto: true,
    hideIfEmpty: ['publications', 'research'],
    requiredSections: ['personal', 'projects', 'skills'],
  },
  'entry-level': {
    id: 'entry-level',
    label: 'Entry Level',
    description: 'Education-first for graduates',
    targetUsers: 'Fresh Graduates, Interns, Career Changers',
    layout: 'single-column',
    sectionOrder: [
      'personal',
      'summary',
      'education',
      'experience',
      'projects',
      'skills',
      'certifications',
      'volunteer',
      'publications',
      'research',
      'awards',
      'languages',
      'interests',
      'references',
      'custom',
    ],
    educationFirst: true,
    showSkillBars: true,
    showsSkillRatingInCv: false,
    showPhoto: false,
    hideIfEmpty: ['publications', 'research'],
    requiredSections: ['personal', 'education', 'skills'],
  },
  healthcare: {
    id: 'healthcare',
    label: 'Healthcare',
    description: 'Licenses, clinical experience focus',
    targetUsers: 'Doctors, Nurses, Allied Health, Therapists',
    layout: 'single-column',
    sectionOrder: [
      'personal',
      'summary',
      'certifications',
      'experience',
      'projects',
      'education',
      'skills',
      'awards',
      'volunteer',
      'publications',
      'research',
      'languages',
      'interests',
      'references',
      'custom',
    ],
    educationFirst: false,
    showSkillBars: false,
    showsSkillRatingInCv: false,
    showPhoto: false,
    hideIfEmpty: ['projects', 'publications'],
    requiredSections: ['personal', 'certifications', 'experience', 'education'],
  },
  'amber-strike': {
    id: 'amber-strike',
    label: 'Amber Strike',
    description: 'Charcoal sidebar, amber accents, creative energy',
    targetUsers: 'Graphic designers, marketers, creative professionals',
    templateAccent: '#F5C518',
    layout: 'two-column',
    sidebarSections: [],
    sectionOrder: [
      'personal',
      'summary',
      'experience',
      'projects',
      'education',
      'skills',
      'certifications',
      'publications',
      'research',
      'awards',
      'volunteer',
      'languages',
      'interests',
      'references',
      'custom',
    ],
    educationFirst: false,
    showSkillBars: true,
    showsSkillRatingInCv: true,
    showPhoto: true,
    hideIfEmpty: ['publications', 'research'],
    requiredSections: ['personal', 'summary', 'experience', 'skills'],
  },
  'midnight-pro': {
    id: 'midnight-pro',
    label: 'Midnight Pro',
    description: 'Deep navy sidebar, electric blue highlights',
    targetUsers: 'Software engineers, data scientists, backend developers',
    templateAccent: '#4F8EF7',
    layout: 'two-column',
    sidebarSections: [],
    sectionOrder: [
      'personal',
      'summary',
      'experience',
      'projects',
      'education',
      'skills',
      'certifications',
      'publications',
      'research',
      'awards',
      'volunteer',
      'languages',
      'interests',
      'references',
      'custom',
    ],
    educationFirst: false,
    showSkillBars: false,
    showsSkillRatingInCv: false,
    showPhoto: true,
    hideIfEmpty: ['publications', 'research', 'volunteer'],
    requiredSections: ['personal', 'summary', 'experience', 'skills'],
  },
  'golden-hour': {
    id: 'golden-hour',
    label: 'Golden Hour',
    description: 'Icon-led sections, timeline experience, golden accents',
    targetUsers: 'Creatives, UX designers, brand managers, artists',
    templateAccent: '#F0B429',
    layout: 'two-column',
    sidebarSections: [],
    sectionOrder: [
      'personal',
      'summary',
      'experience',
      'projects',
      'education',
      'skills',
      'certifications',
      'publications',
      'research',
      'awards',
      'volunteer',
      'languages',
      'interests',
      'references',
      'custom',
    ],
    educationFirst: false,
    showSkillBars: true,
    showsSkillRatingInCv: true,
    showPhoto: true,
    hideIfEmpty: ['publications', 'research'],
    requiredSections: ['personal', 'summary', 'experience', 'skills'],
  },
  'ocean-slate': {
    id: 'ocean-slate',
    label: 'Ocean Slate',
    description: 'Ocean blue sidebar, teal chips and accents',
    targetUsers: 'Product managers, consultants, finance, operations',
    templateAccent: '#00C9A7',
    layout: 'two-column',
    sidebarSections: [],
    sectionOrder: [
      'personal',
      'summary',
      'experience',
      'projects',
      'education',
      'skills',
      'certifications',
      'publications',
      'research',
      'awards',
      'volunteer',
      'languages',
      'interests',
      'references',
      'custom',
    ],
    educationFirst: false,
    showSkillBars: false,
    showsSkillRatingInCv: true,
    showPhoto: true,
    hideIfEmpty: ['publications', 'research'],
    requiredSections: ['personal', 'summary', 'experience', 'skills'],
  },
  'violet-edge': {
    id: 'violet-edge',
    label: 'Violet Edge',
    description: 'Inverted layout, violet header band, timeline main column',
    targetUsers: 'Marketing leads, startup founders, tech creatives, UX leads',
    templateAccent: '#A78BFA',
    layout: 'two-column',
    sidebarSections: [],
    sectionOrder: [
      'personal',
      'summary',
      'experience',
      'projects',
      'education',
      'skills',
      'certifications',
      'publications',
      'research',
      'awards',
      'volunteer',
      'languages',
      'interests',
      'references',
      'custom',
    ],
    educationFirst: false,
    showSkillBars: false,
    showsSkillRatingInCv: true,
    showPhoto: true,
    hideIfEmpty: ['publications', 'research'],
    requiredSections: ['personal', 'summary', 'experience', 'skills'],
  },
  'ats-plain': {
    id: 'ats-plain',
    label: 'ATS Plain',
    description: 'Maximum-compatibility single column with no graphics — safe for any applicant tracking system.',
    targetUsers: 'Anyone applying through ATS-driven job portals',
    layout: 'single-column',
    sectionOrder: [
      'personal',
      'summary',
      'experience',
      'education',
      'skills',
      'projects',
      'certifications',
      'awards',
      'volunteer',
      'languages',
      'publications',
      'research',
      'interests',
      'references',
      'custom',
    ],
    educationFirst: false,
    showSkillBars: false,
    showsSkillRatingInCv: false,
    showPhoto: false,
    hideIfEmpty: ['projects', 'publications', 'research', 'volunteer', 'interests', 'references'],
    requiredSections: ['personal', 'experience', 'education', 'skills'],
  },
  'high-school': {
    id: 'high-school',
    label: 'High School',
    description: 'Education-first layout for students with limited work history.',
    targetUsers: 'High school students applying for first jobs, college, or scholarships',
    layout: 'single-column',
    sectionOrder: [
      'personal',
      'summary',
      'education',
      'volunteer',
      'awards',
      'skills',
      'experience',
      'projects',
      'languages',
      'interests',
      'certifications',
      'publications',
      'research',
      'references',
      'custom',
    ],
    educationFirst: true,
    showSkillBars: false,
    showsSkillRatingInCv: false,
    showPhoto: false,
    hideIfEmpty: ['experience', 'projects', 'publications', 'research', 'certifications', 'references'],
    requiredSections: ['personal', 'education', 'skills'],
  },
  executive: {
    id: 'executive',
    label: 'Executive',
    description: 'Conservative serif layout for senior leadership, with a competencies grid.',
    targetUsers: 'Directors, VPs, and C-suite candidates',
    layout: 'single-column',
    sectionOrder: [
      'personal',
      'summary',
      'skills',
      'experience',
      'education',
      'awards',
      'certifications',
      'volunteer',
      'projects',
      'languages',
      'publications',
      'research',
      'interests',
      'references',
      'custom',
    ],
    educationFirst: false,
    showSkillBars: false,
    showsSkillRatingInCv: false,
    showPhoto: false,
    hideIfEmpty: ['projects', 'publications', 'research', 'volunteer', 'interests'],
    requiredSections: ['personal', 'summary', 'experience'],
  },
  researcher: {
    id: 'researcher',
    label: 'PhD / Researcher',
    description: 'Publication-led academic CV with grants, presentations, and teaching.',
    targetUsers: 'PhD candidates, postdocs, and faculty applicants',
    layout: 'single-column',
    sectionOrder: [
      'personal',
      'summary',
      'education',
      'publications',
      'research',
      'awards',
      'experience',
      'certifications',
      'skills',
      'languages',
      'projects',
      'volunteer',
      'interests',
      'references',
      'custom',
    ],
    educationFirst: true,
    showSkillBars: false,
    showsSkillRatingInCv: false,
    showPhoto: false,
    hideIfEmpty: ['volunteer', 'interests', 'certifications', 'projects'],
    requiredSections: ['personal', 'education', 'publications'],
  },
  europass: {
    id: 'europass',
    label: 'Europass',
    description: 'The standardized EU CV format, including the CEFR language grid.',
    targetUsers: 'Applicants in the EU/EEA, ERASMUS, and EU institutions',
    layout: 'single-column',
    sectionOrder: [
      'personal',
      'summary',
      'experience',
      'education',
      'languages',
      'skills',
      'certifications',
      'projects',
      'volunteer',
      'awards',
      'publications',
      'research',
      'interests',
      'references',
      'custom',
    ],
    educationFirst: false,
    showSkillBars: false,
    showsSkillRatingInCv: false,
    showPhoto: true,
    hideIfEmpty: ['projects', 'publications', 'research', 'awards', 'interests', 'references'],
    requiredSections: ['personal', 'experience', 'education', 'languages'],
  },
};

/** All valid cover letter template IDs — used as allowlist in API routes instead of DB lookup. */
export const CL_TEMPLATE_IDS = [
  'cl-classic',
  'cl-modern',
  'cl-minimal',
  'cl-formal',
  'cl-creative',
] as const;

export type ClTemplateId = (typeof CL_TEMPLATE_IDS)[number];

/** Full catalog of unified template ids (for pickers when DB may be a subset). */
export const ALL_TEMPLATE_IDS: TemplateId[] = [
  'classic',
  'modern',
  'academic',
  'technical',
  'minimal',
  'creative',
  'entry-level',
  'healthcare',
  'amber-strike',
  'midnight-pro',
  'golden-hour',
  'ocean-slate',
  'violet-edge',
  'ats-plain',
  'high-school',
  'executive',
  'researcher',
  'europass',
];

/** All templates visible in the selector. */
export const VISIBLE_TEMPLATE_IDS: TemplateId[] = [
  'classic',
  'modern',
  'academic',
  'technical',
  'entry-level',
  'creative',
  'minimal',
  'healthcare',
  'midnight-pro',
  'ats-plain',
  'high-school',
  'executive',
  'researcher',
  'europass',
];

export function isVisibleTemplate(id: string): boolean {
  return (VISIBLE_TEMPLATE_IDS as readonly string[]).includes(id);
}
