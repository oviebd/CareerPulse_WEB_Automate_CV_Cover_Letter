...
import { AUTH_GOOGLE_ENABLED } from '@/lib/auth-client';
...
export function RegisterForm({ returnTo }: { returnTo?: string }) {
  ...
  const onRegister = async (e: React.FormEvent) => {
    ...
    if (!reg.ok) {
      setLoading(false);
      setError(authErrorMessage(regJson.error) ?? 'Registration failed');
      return;
    }
  ...
  return (
    <div className="w-full max-w-md space-y-8">
      ...
      <form onSubmit={onRegister} className="space-y-4">
        <div>
          <label htmlFor="register-name" className="block text-sm font-medium text-[var(--color-text-primary)]">
            Full name
          </label>
          <input
            id="register-name"
        ...
        <div>
          <label htmlFor="register-email" className="block text-sm font-medium text-[var(--color-text-primary)]">
            Email
          </label>
          <input
            id="register-email"
        ...
        <div>
          <label htmlFor="register-password" className="block text-sm font-medium text-[var(--color-text-primary)]">
            Password
          </label>
          <input
