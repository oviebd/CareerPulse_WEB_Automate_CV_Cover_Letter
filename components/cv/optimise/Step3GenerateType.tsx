...
type Step3GenerateTypeProps = {
  value: GenerationType;
  onChange: (v: GenerationType) => void;
};

export function Step3GenerateType({ value, onChange }: Step3GenerateTypeProps) {
  return (
    <section className="space-y-4">
      <header className="space-y-1">
        <p className="text-xs font-semibold uppercase tracking-wider text-[var(--color-muted)]">Step 3</p>
        <h2 className="font-display text-2xl font-semibold text-[var(--color-text-primary)]">Choose what to generate</h2>
        <p className="text-sm text-[var(--color-muted)]">Select the output for this run.</p>
      </header>
